...
1. **Non-stealth modes default `Epsilon` to 0, not a nonzero default.** ε-greedy exploration (`popNext`'s mid-tier sample) draws from a seeded RNG whose *position in the stream* depends on real dispatch timing under concurrency, not just the seed — turning it on by default for `fast`/`normal`/`quiet` silently broke "same seed → same requests" for a daemon integration test once `-race` slowed scheduling enough to shift draw order. §2's Preset struct comment only mandates "0 in stealth"; it never actually specifies a non-stealth default, so all four modes ship with `Epsilon: 0` and `--epsilon`/`Config.Epsilon` remains an explicit opt-in for coverage-heavy runs that don't need reproducibility.
2. **The AIMD latency-spike trigger has a 50ms floor** (`minLatencyBaselineForSpike`) below which it's skipped entirely. A directory's calibration-baseline median latency against a fast local target (or any low-latency backend) can be sub-millisecond, and the spec's flat 3x-of-baseline multiplier then trips on ordinary scheduling jitter alone — observed stalling several existing integration tests solid. A real WAF slow-path challenge is practically always well above 50ms in absolute terms, so the floor doesn't blunt the signal it's meant to catch.
3. **AIMD state lives inside `httpclient.Limiter` itself**, not a separate controller type: `TriggerBackoff(decrease, now)` / `InBackoff(now, settle)` / `MaybeRecover(step, window, now)` replace the old one-shot `TriggerBackoff(factor, window, now)` / `InBackoff(now)`. `SessionState`'s `BackoffMultiplier`/`BackoffUntil` became `AIMDRate`/`AIMDLastTrigger`/`AIMDTriggered` accordingly (spec §6 "wafState," upgraded shape, not a `SessionVersion` bump — an old snapshot just resumes with the backoff state reset to clean).
4. Everything else — `Preset`/`ResolvePreset`, `JitterSpec` distributions, header profiles + stable identity + referer chains, `OrderJitter`'s priority-band shuffle, the `HTTPDoer` boundary, time-budget pacing, and the §7 statistical rate-rigor test — matches this spec as written. `go build`, `go vet`, and `go test -race ./...` are clean.
